...
// codegraphToolWhitelist mirrors the importable functions exposed by the
// better_code_review_graph.tools module to the Python shim.
var codegraphToolWhitelist = []string{
...
func NewCodegraphTool(workingDir string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		...
		func(ctx context.Context, params CodegraphParams, _ fantasy.ToolCall) (fantasy.ToolResponse, error) {
			tool := params.Tool
			valid := false
			for _, allowed := range codegraphToolWhitelist {
				if tool == allowed {
					valid = true
					break
				}
			...
			if !valid {
				msg := fmt.Sprintf("unknown tool %q (allowed: %s)", tool, strings.Join(codegraphToolWhitelist, ", "))
